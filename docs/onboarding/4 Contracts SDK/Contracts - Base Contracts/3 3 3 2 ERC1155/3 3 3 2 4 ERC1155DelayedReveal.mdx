---
slug: /contractkit/base-contracts/erc-1155/erc1155delayedreveal
title: Delayed Reveal
---

import ViewContractCodeButton from "@components/ViewContractCodeButton";
import QuickstartCard from "@components/QuickstartCard";
import ImportSnippet from "@components/ImportSnippet";

# ERC1155 Delayed Reveal

`ERC1155DelayedReveal` adds the [Delayed Reveal](/sdk/interacting-with-contracts/erc1155delayedreveal) feature to the
[ERC1155LazyMint](/contractkit/base-contracts/erc-1155/erc1155lazymint) base contract.

<div style={{ marginBottom: 24 }}>
  <ViewContractCodeButton
    link="https://github.com/web3sdkio/contracts/blob/main/contracts/base/ERC1155DelayedReveal.sol"
    name="ERC1155DelayedReveal"
  />
</div>

Delayed reveal allows you to distribute NFTs to your audience with placeholder metadata, and reveal the real metadata of the distributed NFTs at a later time using a password.

<div className="row" style={{ marginBottom: 24 }}>
  <div className="col col--12" style={{ marginTop: 8 }}>
    <QuickstartCard
      name="Learn more about delayed reveal"
      link="https://blog.web3sdk.io/delayed-reveal-nfts"
      image="/assets/icons/sdks.png"
    />
  </div>
</div>

## Unlocked Features

Once deployed, you can use the features made available by these contracts on the SDK and dashboard:

Click on each feature to learn more about what functions are available.

<div className="row" style={{ marginBottom: 24, flexWrap: "wrap" }}>
  <div className="col col--4" style={{ padding: 8 }}>
    <QuickstartCard
      name="ERC1155 Standard"
      link="/sdk/interacting-with-contracts/erc1155"
      image="/assets/icons/edition.png"
    />
  </div>
  <div className="col col--4" style={{ padding: 8 }}>
    <QuickstartCard
      name="Lazy Minting"
      link="/sdk/interacting-with-contracts/erc1155lazymintable"
      image="/assets/icons/edition-drop.png"
    />
  </div>
  <div className="col col--4" style={{ padding: 8 }}>
    <QuickstartCard
      name="Claimable"
      link="/sdk/interacting-with-contracts/erc1155claimable"
      image="/assets/icons/edition-drop.png"
    />
  </div>
  <div className="col col--4" style={{ padding: 8 }}>
    <QuickstartCard
      name="Delayed Reveal"
      link="/sdk/interacting-with-contracts/erc1155delayedreveal"
      image="/assets/icons/edition-drop.png"
    />
  </div>
  <div className="col col--4" style={{ padding: 8 }}>
    <QuickstartCard
      name="Burnable"
      link="/sdk/interacting-with-contracts/erc1155burnable"
      image="/assets/icons/edition.png"
    />
  </div>
  <div className="col col--4" style={{ padding: 8 }}>
    <QuickstartCard
      name="Contract Metadata"
      link="/sdk/advanced-features/contract-settings"
      image="/assets/icons/general.png"
    />
  </div>
  <div className="col col--4" style={{ padding: 8 }}>
    <QuickstartCard
      name="Ownable"
      link="/contractkit/extension-contracts/ownable"
      image="/assets/icons/general.png"
    />
  </div>
  <div className="col col--4" style={{ padding: 8 }}>
    <QuickstartCard
      name="Royalties"
      link="/sdk/advanced-features/contract-settings#royalty-fees"
      image="/assets/icons/general.png"
    />
  </div>
</div>

## Implementing the Contract

Import the contract extension and make your contract inherit it.

```solidity
// SPDX-License-Identifier: Apache-2.0
pragma solidity ^0.8.0;

import "@web3sdkio/contracts/base/ERC1155DelayedReveal.sol";

contract MyNFT is ERC1155DelayedReveal {
    constructor(
        string memory _name,
        string memory _symbol,
        address _royaltyRecipient,
        uint128 _royaltyBps
    ) ERC1155DelayedReveal(_name, _symbol, _royaltyRecipient, _royaltyBps) {}
}
```
