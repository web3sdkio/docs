...
We'll start by cloning the [Web3sdkio Auth Next](https://github.com/web3sdkio-template/web3sdkio-auth-next) example project using the [web3sdkio CLI](/cli)

```bash
npx web3sdkio@latest create --template web3sdkio-auth-next
```
...
1. `/api/auth/[...web3sdkio].ts`: A catch-all route that handles `/login`, `/logout`, and `/user` requests.
...
In the `_app.tsx` file, we have our application wrapped in the `Web3sdkioProvider` with an `authConfig` option.
...
- [`/api/auth/login`](https://github.com/web3sdkio/web3/blob/main/packages/auth/src/next/routes/login.ts): Verifies the user's signature and generates an auth token that gets set in the user's cookies.
- [`/api/auth/logout`](https://github.com/web3sdkio/web3/blob/main/packages/auth/src/next/routes/logout.ts): Deletes the cookie containing the auth token.
- [`/api/auth/user`](https://github.com/web3sdkio/web3/blob/main/packages/auth/src/next/routes/user.ts): Reads the user's cookie, authenticates its integrity and returns the user's wallet address.

All we need to do is configure the catch-all API route that handles these requests.

```ts title="/api/auth/[...web3sdkio].ts"
...
By exporting the `Web3sdkioAuthHandler`, we tell the SDK to handle requests to any `/api/auth/...` route.
...
import { useAddress, useLogin, ConnectWallet } from "@web3sdkio/react";
...
```
web3sdk.io wants you to sign in with your account:
...
`loginRedirect` option in the `Web3sdkioProvider`; and are now authenticated!

:::tip

For those curious, you can view the authentication token by right-clicking the page > `Inspect` > `Application` tab > `Cookies` and you'll see it under the `web3sdkio_auth_token` cookie in this case.
...
in the `Web3sdkioProvider` using the `useUser` hook:
...
<div className="row" style={{ marginTop: 24 }}>
  <div className="col col--6" style={{ padding: 8 }}>
    <QuickstartCard
      ...
      link="https://github.com/web3sdkio-template/nft-gated-website"
  ...
  <div className="col col--6" style={{ padding: 8 }}>
    <QuickstartCard
      ...
      link="https://github.com/web3sdkio-template/web3sdkio-stripe"
