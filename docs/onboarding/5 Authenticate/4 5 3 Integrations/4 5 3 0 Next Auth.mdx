...
<div className="row" style={{ marginTop: 24 }}>
  <div className="col col--12">
    <QuickstartCard
      ...
      description="Integrate web3sdkio Auth as a NextAuth Provider"
...
First, we need to install the `@web3sdkio/auth` and `next-auth` packages as follows:

```bash npm2yarn
npm install @web3sdkio/auth next-auth
```
...
Here, we configure Auth with the `Web3sdkioNextAuth` function, passing in our private key and domain (used to prevent phishing attacks).
...
Now, in the `_app.js` file, we'll need to wrap our app with the `Web3sdkioProvider` and provide some configuration for Auth, as well as the Next Auth `SessionProvider`:
...
export default function MyApp({
  ...
  return (
    <SessionProvider session={session}>
      <Web3sdkioProvider desiredChainId={1}>
        <Component {...pageProps} />
      </Web3sdkioProvider>
    </SessionProvider>
  );
...
```jsx
import { useSDK, useAddress, useMetamask } from "@web3sdkio/react";
...
<div className="col col--6" style={{ padding: 8 }}>
  <QuickstartCard
    name="Auth + Next Auth"
    link="https://github.com/web3sdkio-template/web3sdkio-auth-next-auth"
    description="Implement web3sdkio Auth with Next Auth"
