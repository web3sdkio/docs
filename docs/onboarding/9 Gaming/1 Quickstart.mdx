...
### Installing web3sdkio SDK

Let's import the web3sdkio SDK so we can add some web3 functionality!

First, head over to the [`web3sdkio/unity-sdk` releases page](https://github.com/web3sdkio/unity-sdk/releases) on GitHub.
...
From the `Scenes` folder, double-click the `Web3sdkioDemoScene.unity` file to open it. If you manoeuvre around the scene, you'll be able to see the `Canvas` that comes as part of the demo:

![image.png](./assets/7.png)

To view the code powering this demo and see what you can do with the SDK, open the `Web3sdkioSDKDemos.cs` file by double-clicking it:
...
// Import the Web3sdkio SDK namespace
using Web3sdkio;

public class Web3sdkioSDKDemos : MonoBehaviour
{
    // Create a Web3sdkio SDK instance for us to use throughout this class
    private Web3sdkioSDK sdk;

    void Start()
    {
        // When the app starts, set up the Web3sdkio SDK
...
Next, click `Player Settings...` (bottom left corner) and set the `WebGL Template` under `Resolution and Presentation` to `Web3sdkio`:
...
We can then use [Storage](https://portal.web3sdk.io/storage) to upload and deploy our application to IPFS, by running the following command:

```bash
npx web3sdkio@latest upload .
```
