...
Inside the [`index.js`](https://github.com/web3sdkio-template/cra-javascript-starter/blob/main/src/index.js#L15-L17) file,
  ...
  return (
    <>
      {isReadingNfts ? (
        ...
        <div>
          {nfts?.map((nft) => (
            <Web3sdkioNftMedia metadata={nft.metadata} key={nft.metadata.id} />
          ))}
        </div>
      )}
    </>
...
import { Web3Button } from "@web3sdkio/react";
...
To [host your application on IPFS](https://blog.web3sdk.io/guides/how-to-host-your-web-app-on-ipfs/)
...
<TwitterCard
  ...
  text={`Probably nothing 👀 @web3sdkio
