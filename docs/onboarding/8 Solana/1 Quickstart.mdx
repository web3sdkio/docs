...
Follow our [Getting Started with Solana guide](https://blog.web3sdk.io/guides/getting-started-with-solana/) to create a wallet and get test funds!
...
const sdk = Web3sdkioSDK.fromPrivateKey("devnet", privateKey);
```
...
function MyApp({ Component, pageProps }: AppProps) {
  return (
    <Web3sdkioProvider endpoint={network}>
      <WalletModalProvider>
        <Component {...pageProps} />
      </WalletModalProvider>
    </Web3sdkioProvider>
...
And just like that, we've deployed a new NFT Drop to Solana, lazy-minted some NFTs, and created a claiming website, all with the web3sdkio Solana SDK!
