---
slug: /solana/react.usetransfernft
title: useTransferNFT() function
hide_title: true
displayed_sidebar: react
---

<!-- Do not edit this file. It is automatically generated by API Documenter. -->

## useTransferNFT() function

Transfer NFTs from the connected wallet to another wallet

## Example

```jsx
import { useProgram, useTransferNFT } from "@web3sdkio/react/solana";

export default function Component() {
  const { program } = useProgram("{{program_address}}");
  const { mutateAsync: transfer, isLoading, error } = useTransferNFT(program);

  return (
    <button
      onClick={() =>
        transfer({
          receiverAddress: "{{wallet_address}}",
          tokenAddress: "...",
        })
      }
    >
      Transfer
    </button>
  );
}
```

**Signature:**

```typescript
export declare function useTransferNFT(
  program: RequiredParam<NFTCollection | NFTDrop>,
): import("@tanstack/react-query").UseMutationResult<
  import("@web3sdkio/sdk/solana").TransactionResult,
  unknown,
  TransferNFTMutationParams,
  unknown
>;
```

## Parameters

| Parameter | Type                                              | Description                                  |
| --------- | ------------------------------------------------- | -------------------------------------------- |
| program   | RequiredParam&lt;NFTCollection &#124; NFTDrop&gt; | The NFT program instance to transfer NFTs on |

**Returns:**

import("@tanstack/react-query").UseMutationResult&lt;import("@web3sdkio/sdk/solana").TransactionResult, unknown, [TransferNFTMutationParams](./react.transfernftmutationparams.md), unknown&gt;
